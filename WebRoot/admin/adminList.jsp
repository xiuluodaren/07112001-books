...
function frist()
{
	 ...
	 with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/AdminServlet?mode=list&pageNoStr=1";
            submit();
        }
}
...
function back()
{
    ...
    if(pageNo<2){
 	   ...
 	   pageNo=Number(pageNo)-1;
		 ...
		 with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/AdminServlet?mode=list&pageNoStr="+pageNo;
	            submit();
	        }
}
...
{ 
	   var totlePage='<%=totlePage%>';
		with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/AdminServlet?mode=list&pageNoStr="+totlePage;
            submit();
        }
	}
...
function add() {
    window.self.location = "admin/adminAdd.jsp";
}
...
function modify() {
    ...
    if (count == 1) {
       window.self.location = "admin/adminModify.jsp?id="+j;
    }
}
...
function deletes() {
    ...
    if (window.confirm("确认删除吗？")) {
        with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/AdminServlet?mode=deletes";
            submit();
        }
    }
}
            ...
            <tr>
              <td width="60"><table width="87%" border="0" cellpadding="0" cellspacing="0">
                  <tr onClick="checkAll()" >
                    <td class="STYLE1"><div align="center">
                       <input type="checkbox" id="ifAll" value="checkbox"/>
                    </div></td>
              ...
              <td width="52"><table width="88%" border="0" cellpadding="0" cellspacing="0">
                  <tr onclick="modify()" >
                    ...
                    <td class="STYLE1"><div align="center">修改口令</div></td>
                  </tr>
              </table></td>
              <td width="52"><table width="88%" border="0" cellpadding="0" cellspacing="0">
                  <tr onclick="deletes()" >
