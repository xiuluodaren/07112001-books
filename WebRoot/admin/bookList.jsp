...
function frist()
{
	 ...
	 with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/BookServlet?mode=list&pageNoStr=1";
            submit();
        }
}
...
function back()
{
    ...
    if(pageNo<2){
 	   ...
 	   pageNo=Number(pageNo)-1;
		 ...
		 with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/BookServlet?mode=list&pageNoStr="+pageNo;
	            submit();
	        }
}
...
{ 
	   var totlePage='<%=totlePage%>';
		with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/BookServlet?mode=list&pageNoStr="+totlePage;
            submit();
        }
	}
...
function add() {
    window.self.location = "admin/bookAdd.jsp";
}
...
function deletes() {
    ...
    if (window.confirm("确认删除吗？")) {
        with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/BookServlet?mode=deletes";
            submit();
        }
    }
}
...
function search(){
        with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/BookServlet?mode=list";
            submit();
        }
}
</script>
