...
function frist()
{
	 ...
	 with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/BookTypeServlet?mode=list&pageNoStr=1";
            submit();
        }
}
...
function back()
{
    ...
    if(pageNo<2){
 	   ...
 	   pageNo=Number(pageNo)-1;
		 ...
		 with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/BookTypeServlet?mode=list&pageNoStr="+pageNo;
	            submit();
	        }
}
...
{ 
	   var totlePage='<%=totlePage%>';
		with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/BookTypeServlet?mode=list&pageNoStr="+totlePage;
            submit();
        }
	}
...
function add() {
    window.self.location = "admin/booktypeAdd.jsp";
}
...
function modify() {
    ...
    if (count == 1) {
        with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/BookTypeServlet?mode=modifyBefore";
            submit();
        }
    }
}
...
function deletes() {
    ...
    if (window.confirm("确认删除吗？")) {
        with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/BookTypeServlet?mode=deletes";
            submit();
        }
    }
}
        ...
        <td height="22" bgcolor="#FFFFFF"><div align="center"><strong><span class="STYLE1">类型名称</span></strong></div></td>
         <td height="22" bgcolor="#FFFFFF"><div align="center"><strong><span class="STYLE1">可借天数</span></strong></div></td>
			...
			for(int i = 0;i<list.size();i++){
				BookType bean = (BookType)list.get(i);
			%>
