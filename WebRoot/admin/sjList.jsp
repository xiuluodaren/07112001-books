...
function frist()
{
	 ...
	 with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/SjServlet?mode=list&pageNoStr=1";
            submit();
        }
}
...
function back()
{
    ...
    if(pageNo<2){
 	   ...
 	   pageNo=Number(pageNo)-1;
		 ...
		 with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/SjServlet?mode=list&pageNoStr="+pageNo;
	            submit();
	        }
}
...
{ 
	   var totlePage='<%=totlePage%>';
		with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/SjServlet?mode=list&pageNoStr="+totlePage;
            submit();
        }
	}
...
function add() {
    window.self.location = "admin/sjAdd.jsp";
}
...
function modify() {
    ...
    if (count == 1) {
        with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/SjServlet?mode=modifyBefore";
            submit();
        }
    }
}
...
function deletes() {
    ...
    if (window.confirm("确认删除吗？")) {
        with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/SjServlet?mode=deletes";
            submit();
        }
    }
}
