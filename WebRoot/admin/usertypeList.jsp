...
function frist()
{
	 ...
	 with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/UserTypeServlet?mode=list&pageNoStr=1";
            submit();
        }
}
...
function back()
{
    ...
    if(pageNo<2){
 	   ...
 	   pageNo=Number(pageNo)-1;
		 ...
		 with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/UserTypeServlet?mode=list&pageNoStr="+pageNo;
	            submit();
	        }
}
...
{ 
	   var totlePage='<%=totlePage%>';
		with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/UserTypeServlet?mode=list&pageNoStr="+totlePage;
            submit();
        }
	}
...
function add() {
    window.self.location = "admin/usertypeAdd.jsp";
}
...
function modify() {
    ...
    if (count == 1) {
        with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/UserTypeServlet?mode=modifyBefore";
            submit();
        }
    }
}
...
function deletes() {
    ...
    if (window.confirm("确认删除吗？")) {
        with (document.getElementById("memberForm")) {
            method = "post";
            action = "<%=path %>/UserTypeServlet?mode=deletes";
            submit();
        }
    }
}
    ...
    <td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#c9c9c9">
      <tr>
       <td height="22" bgcolor="#FFFFFF"><div align="center"><strong><span class="STYLE1"></span></strong></div></td>
        <td height="22" bgcolor="#FFFFFF"><div align="center"><strong><span class="STYLE1">类型名称</span></strong></div></td>
         <td height="22" bgcolor="#FFFFFF"><div align="center"><strong><span class="STYLE1">可借天数</span></strong></div></td>
      </tr>
      		<%
			for(int i = 0;i<list.size();i++){
				UserType bean = (UserType)list.get(i);
			%>
      ...
      <tr>
       <td height="22" bgcolor="#FFFFFF"><div align="center"><span class="STYLE3"><input type="checkbox" name="selectFlag" id="selectFlag" class="checkbox1"
        ...
        <td height="22" bgcolor="#FFFFFF"><div align="center"><span class="STYLE3"><%=bean.getNames() %></span></div></td>
         <td height="22" bgcolor="#FFFFFF"><div align="center"><span class="STYLE3"><%=bean.getKj() %></span></div></td>
      </tr>
      <%} %>
